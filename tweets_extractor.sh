...
then
    ...
    start_time=`date '+%Y-%m-%d %H:%M:%S'`    
    echo "${running_date},${COLLECTION_NAME},'index_tweets',${start_time}," >> $EVENT_LOG
...
then
    ...
    start_time=`date '+%Y-%m-%d %H:%M:%S'`    
    echo "${running_date},${COLLECTION_NAME},'learn_embeddings',${start_time}," >> $EVENT_LOG
...
then
    end_time=`date '+%Y-%m-%d %H:%M:%S'`
    echo "${running_date},'learn_embeddings',,${end_time}" >> $EVENT_LOG    
else
    error=1
fi
